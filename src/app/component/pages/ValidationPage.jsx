  ...
  const tabs = [
    {
      ...
      desc: isAr ? "تحقق من صحة شهادات المنظمات والشركات" : "Verify organization & business certificates",
    },
    {
      id: "individual",
      ...
      desc: isAr ? "تحقق من صحة شهادات الأفراد والمهنيين" : "Verify personal & professional certificates",
    },
  ];
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                <span className={`${isAr ? "font-heading" : "font-display"} text-[0.9rem] sm:text-[0.95rem] font-bold leading-tight transition-colors duration-300
...
function SearchForm({ type, lang, onResult }) {
  ...
  const placeholder = isCompany
    ? (isAr ? "أدخل رمز شهادة الشركة..." : "Enter company certificate code...")
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={handleSearch} className="validation-search-form">
        <div className={`validation-input-wrap ${isAr ? "font-heading" : "font-display"}`}>
          <svg className="validation-search-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        >
          {loading ? (
            ...
            <>
              <svg className="validation-btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="11" cy="11" r="8"/>
                <path d="m21 21-4.35-4.35"/>
              </svg>
              {isAr ? "بحث" : "Search"}
      ...
      {loading && (
        <div className="validation-loading">
          ...
          <p>{isAr ? "جاري البحث عن شهادتك…" : "Searching for your certificate…"}</p>
        </div>
      )}
      ...
      {searched && !loading && !result && (
        <div className="validation-empty">
          <svg className="validation-empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <>
      ...
      >
        ...
        <div className="relative z-10 text-center px-6 py-20 max-w-4xl mx-auto">
          <h2 className={`${isAr ? "font-heading" : "font-display"} text-[clamp(1.75rem,3.5vw,2.75rem)] font-bold text-blue-400 leading-[1.2] mb-4 drop-shadow-lg`}>
      ...
      <Section>
        <Container>
          ...
          <div className="flex flex-col items-center mb-10">
            <span className={`inline-block ${isAr ? "font-heading" : "font-display"} text-[0.72rem] font-bold tracking-[0.08em] uppercase text-[var(--primary-color)] bg-[rgba(55,118,189,0.08)] px-4 py-2 rounded-full mb-4`}>
              {valid}
            </span>
            <h2 className={`${isAr ? "font-heading" : "font-display"} text-[clamp(1.4rem,3vw,2rem)] font-bold text-[#0f172a] leading-[1.2] text-center mb-2`}>
              {sub}
            </h2>
          ...
          <div className="flex items-center gap-4 max-w-2xl mx-auto mb-8">
            ...
            <span className={`${isAr ? "font-heading" : "font-display"} text-[0.72rem] font-semibold text-[#94a3b8] uppercase tracking-wider whitespace-nowrap`}>
              {type === "company"
                ? (isAr ? "أدخل رمز شهادة الشركة" : "Enter company code")
                : (isAr ? "أدخل رمز الشهادة الشخصية" : "Enter individual code")}
            </span>
            <div className="flex-1 h-[0.5px] bg-[#e2e8f0]" />
          </div>
